...
void display() {
    ...
    glEnd();
    //drawing the picking dots 
    glDisable(GL_LIGHTING);
    glPointSize(20.0f); // Bigger dots
    glBegin(GL_POINTS);
    for (size_t idx = 0; idx < pickedIDs.size(); idx++) {
        int id = pickedIDs[idx];
        int i = id * 3;
...
void displayGlobalView(){
    ...
    glPointSize(20.0f); // Bigger dots
    glBegin(GL_POINTS);
    for (size_t idx = 0; idx < pickedGlobalIDs.size(); idx++) {
        int id = pickedGlobalIDs[idx];
        int i = id * 3;
...
void mouseClick(int button, int state, int x, int y) {
    ...
    if (glutGetWindow() == mainWindow && button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        if (id >= 0 && std::find(pickedIDs.begin(), pickedIDs.end(), id) == pickedIDs.end()) {
            ...
            //save the 2d location of the picking for ePnP
            int i = id * 3;
            MyVec3f v0 = vertices[indices[i]];
            MyVec3f v1 = vertices[indices[i + 1]];
            MyVec3f v2 = vertices[indices[i + 2]];
    ...
    //picking in the global window
    else if (glutGetWindow() == globalWindow && button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        GLint viewport[4];
        glGetIntegerv(GL_VIEWPORT, viewport);
        ...
        // Add the picked id only if valid, not already picked and the number of picked global ids is not larger then the number of picked ids
        if (id >= 0 && std::find(pickedGlobalIDs.begin(), pickedGlobalIDs.end(), id) == pickedGlobalIDs.end() && pickedIDs.size() > pickedGlobalIDs.size()) {
            pickedGlobalIDs.push_back(id);
